...
--
-- SLIM C code configuration and generation
...
data Config = Config
  { -- | Alternative primary function name.  If this is empty, then it will
    -- default to the name passed to 'Language.SLIM.C.Compile.compile'.
    cFuncName     :: String
...
-- @clockName@) the first time you enter the main SLIM-generated function
-- calling your rules.
data Clock = Clock
  ...
  where
    ...
    c = unlines
      ...
      , codeAssertionChecks mp config assertNames coverNames rules
        ...
        -- generate "main" (at least as far as SLIM is concerned)
      , "void " ++ funcName ++ "()"
      , "{"
      , unlines [ swOrHwClock
                , codePeriodPhases
...
-- | Generate C assertions for each SLIM assertion
